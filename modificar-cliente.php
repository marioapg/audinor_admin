...
<title>Audinor - Modificar Cliente</title>
...
<body>
  ...
  <div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">     
    ...
    $query = "SELECT full_name
    				...
    				,acompanante
    				,tipo_adaptacion
    				FROM cliente WHERE clave='".$_GET['id']."' AND sede='".$_GET['sede']."';";
    
    $sedeori = $_GET['sede'];
    $idcliente = $_GET['id'];
    $resultado = mysql_query($query) or die("ERROR CONSULTANDO LOS CLIENTES");
  ...
  <button type="submit" onclick="retroceder();" class="btn btn-primary glyphicon glyphicon-circle-arrow-left"></button>
    ...
    <form action="updatecliente.php" method="post">
        <p>Nombre Completo:
        <input type="text" name="nombre" size="30" value="<?php echo $row['full_name']; ?>" required/>
        Dirección:
        <input type="text" name="direccion" size="50" value="<?php echo $row['direccion']; ?>" required/></p>
        <p>Municipio:
        <input type="text" name="municipio" size="25" value="<?php echo $row['municipio']; ?>" required/>
        Provincia:
        <input type="text" name="provincia" size="25"  value="<?php echo $row['provincia']; ?>" required/>
        Codigo Postal:
        <input type="number" name="codpostal" size="10" value="<?php echo $row['codigo_postal']; ?>" required/> </p>
        Sede:
        <?php 
        ...
        <?php 
         ...
         ?>
        <p>Teléfono:
        <input type="text" name="telefono" size="25" value="<?php if($row['telefono'] != "NULL") echo $row['telefono'];else echo "0";?>" required/>
        e-mail:
        <input type="email" name="email" size="25" value="<?php if($row['email'] != "NULL") echo $row['email'];else echo "0";?>" required/></p>
        ¿Publicidad?:
        <?php 
           	if ($row['publicidad']=='0'){
        ?>
        ...
        Profesión:
        <input type="text" name="profesion" size="30" value="<?php if($row['profesion'] != "NULL") echo $row['profesion'];else echo "0";?>" required/></p>
        <p>Observaciones:
        ...
        <input type="text" value="<?php echo $idcliente;?>" name="idcliente" style="display: none;"/>
        <input type="text" value="<?php echo $sedeori;?>" name="sedeori" style="display: none;"/>
