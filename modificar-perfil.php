...
<title>Audinor - Actualizar Perfil</title>
...
<body>
  ...
  <button type="submit" onclick="retroceder();" class="btn btn-primary glyphicon glyphicon-circle-arrow-left"></button>
    ...
    <?php 
	$consulta = "SELECT id,full_name,username,email,password FROM usertbl WHERE username='".$_SESSION['session_username']."';";
	$resultado1 = mysql_query($consulta) or die("ERRO Buscando Admin");
	$row = mysql_fetch_array($resultado1);
	?>
    <form action="updateperfil.php?id=<?php echo $row['id']; ?>" method="post">
        <p>Nombre Completo:
        <input type="text" name="nombre" size="30" value="<?php echo $row['full_name']; ?>" required/></p>
        <p>Username:
        <input type="text" name="username" size="15" value="<?php echo $row['username']; ?>" required/></p>
        <p>Email:
        <input type="email" name="email" size="25" value="<?php echo $row['email']; ?>" required/></p>
        <p>Password:
        <input type="password" name="password" size="25"  value="<?php echo $row['password']; ?>" required/>
        </p>    
